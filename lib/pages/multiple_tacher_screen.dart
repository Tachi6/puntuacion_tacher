...
class MultipleTacherScreen extends StatelessWidget {
  const MultipleTacherScreen({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return TacherScreen(
      ...
      bottomSheet: CustomBottomSheet(
        ...
        child: SendTasteButton(isWineTasted: multipleProvider.isWineTasted(selectedWineTaste?.id ?? '-1')), 
        ),
      ),
...
class SendTasteButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomElevatedButton(
      width: 120,
      label: isWineTasted ? 'Salir' : 'Valorar',
